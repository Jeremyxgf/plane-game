...
class Enemy(DestroyAnimationMixin):
    def __init__(self, image_path=os.path.join(source_dir, 'enemy1.png'), speed=4000, background_size=(480, 700)):
        '''
...
# 飞机子弹类
class Bullet():
    def __init__(self, image_path=os.path.join(source_dir,'bomb.png'), background_size=(480, 700), plan=None, speed=1000):
        '''
...
class Plan(DestroyAnimationMixin):
    def __init__(self, image_path=os.path.join(source_dir,'plan.png'), background_size=(480, 700)):
        ...
        # 飞机每次移动的距离
        self.every_time_move_distance = 2
        # 飞机的子弹
        self.bullets = []
...
class Game():
    def __init__(self, background_image_path, size=(480, 700), title='飞机大战', font_name='方正舒体', font_size=30, speed=4000):
        '''
    ...
    def set_time_passed(self):
        ...
        # 得到上一次绘制图像到到现在的时间, ms
        self.time_passed = self.clock.tick_busy_loop(100)
        #self.time_passed = self.clock.tick(200)
        # self.time_passed = self.clock.tick()
        # print (self.time_passed)
        # Jeremy
        '''
        self.overalltime += self.time_passed
        self.overallcounter += 1
        if self.overalltime >= 1000:
            print (self.overallcounter)
            self.overalltime = 0
            self.overallcounter = 0
        '''
    ...
    def plan_and_enemy_crash_detection(self, plan, allow_crash_size=None):
        ...
        for enemy in self.enemies:
            # 遍历每一个敌机， 检测是否碰撞
            if enemy.start_destroy or enemy.destroyed:
                continue
